...
\begin{longtable}{L{0.1\textwidth} L{0.12\textwidth} L{0.7\textwidth}}
	\textbf{Nr.} & \textbf{Prio}  & \textbf{Description} \\
	\reqRow{res}{1}{Must}{The system recognizes failures within an hour and recovers from them without the \qos or the functionality of the system being affected.} % GK: split in two requirements. reduce to detect sensor failures within half an hour
	\reqRow{res}{2}{Must}{The system continues to function with the same \qos in a situation where up to 10\% of the sensors suffer from failures.} % WM: is 5% realistic?
	\bottomrule
\end{longtable}
...
\begin{longtable}{L{0.1\textwidth} L{0.12\textwidth} L{0.7\textwidth}}
	...
	\reqRow{intr}{4}{Must}{When the system detects a high risk of flood, it warns emergency services automatically using the API provided by the emergency services.} % GK: change it guidance for emergency service
...
\begin{longtable}{L{0.1\textwidth} L{0.12\textwidth} L{0.7\textwidth}}
	...
	\reqRow{sec}{4}{Must}{All communication to, from and within the system are encrypted.}
	...
	\reqRow{sec}{7}{Must}{Backup copies are stored in a secure location which is not in the same area as the system.} % WM: maybe make area more specific, like range of 50 km or something
