...
\begin{longtable}{L{0.1\textwidth} L{0.12\textwidth} L{0.7\textwidth}}
	...
	\reqRow{rel}{Must}{Data from the sensors is sent via a TCP connection} % Already mention REST here?
...
\begin{longtable}{L{0.1\textwidth} L{0.12\textwidth} L{0.7\textwidth}}
	\textbf{Nr.} & \textbf{Prio}  & \textbf{Description} \\
	\reqRow{intr}{Must}{The system pulls weather forecasts from at least two weather forecasting services.}
	...
	\reqRow{intr}{Must}{The systems is able to connect to different types of sensors.}
	\reqRow{intr}{Must}{The system is able to retrieve geographical data from an API.}
	\reqRow{intr}{Must}{The system exposes an API, allowing third parties to develop applications using the systems data.}
	\reqRow{intr}{Must}{The sensors are able to communicate with the system using a mobile broadband connection.}
...
\begin{longtable}{L{0.1\textwidth} L{0.12\textwidth} L{0.7\textwidth}}
	...
	\reqRow{sec}{Must}{All communication to, from and within the system are encrypted.}
	\reqRow{sec}{Must}{User account information is hashed using bcrypt after being salted with 128 randomly generated characters.}
	\reqRow{sec}{Must}{The system is protected by a firewall that at least scans at the application layer, while also scanning for and mitigating DDoS attacks.}
	% WM: does communication within the system mean between the different components of the system?
	\reqRow{sec}{Must}{The system communicates with its sensors via a REST API that only allows for HTTPS connection.} % Do we want to communicate with the sensors via REST? WM: Could this be more generic? Like: 'the system communicates with the sensors via a secure connection'
...
\begin{longtable}{L{0.1\textwidth} L{0.12\textwidth} L{0.7\textwidth}}
	\textbf{Nr.} & \textbf{Prio}  & \textbf{Description} \\
	\reqRow{scale}{Must}{ The database and services of the system should run in a cloud environment where they can scale, when the systems resource usage increases. }
