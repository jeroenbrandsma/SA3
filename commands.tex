...
\newcommand{\nextNrRef}[1]{%
\getNext{#1}\manuallabel{#1:\arabic{#1}}{\arabic{#1}}\arabic{#1}
}
...
\newcommand{\hlReqRow}[3]{%
	\midrule
	\\
	\phantomsection
	\getNext{HL}
	\manuallabel{hl:#1}{HL-\arabic{HL}} 	
	HL-\arabic{HL} &
	\ifcase\pdfstrcmp{#2}{Must}%
	   \textbf{#2}
	\else
...
\newcommand{\risk}[6]{%
\begin{tabular}{L{0.3\textwidth} L{0.7\textwidth}}\toprule\phantomsection\getNext{#1-RISK}\manuallabel{#1-RISK\arabic{#1-RISK}}{#1-RISK\arabic{#1-RISK}}\textbf{#1-RISK\arabic{#1-RISK}} & \textbf{#2} \\*
\midrule
