...
This section will give an overview of the different use-cases. Figure \ref{fig:usecase-diagram} displays the use-case diagram. In this diagram, we introduce the «after» notation to indicate that one use-case occurs after another use case in time. This provides an overview of the use-cases with their actors. In the subsections below, the architectural important use-cases are explained in more detail. The use-case diagram is made using the UML notation. The UML notation is extended with after notation. This means that this use-case will happen after the connected use-case.
...
\caption{UC-\arabic{uc}: Receive sensor data}
\label{table:uc-receive-sensordata}
\end{table}
...
\pgfplotstabletypeset[%
	...
	value & description \\
	Number & \req{uc}\\
	Description & The system receives data from the weather forecast service, geographic data form the geographic API and demographic data from the demographic API \\
	Stakeholders and interests & 
	\textbf{Developers}: Developers would like to have a simple to use API \\
	Primary actor & System\\
	Scope & Monitoring part of the system \\
	Level & Sub process\\
	Precondition & The system needs some external data to predict floods \\
	Main success scenario & \compactList{enumerate}{%
		\item The processing unit determines it needs external data%
		\item A call is made to the relevant extrnal API%
	...
	Related requirements & \ref{fr:receive-weather}, \ref{fr:receive-geographic}, \ref{fr:compute-nrcivilians}\\
}
\caption{UC-\arabic{uc}: Receive weather/GEO data}
...
\pgfplotstabletypeset[%
	...
	Stakeholders and interests & \compactList{itemize}{%
		...
		\item \textbf{Citizens}: The citizens want to be alerted if there is a high enough probability of a flood occuring
		}\\
	Primary actor & System\\
	...
	Main success scenario & \compactList{enumerate}{%
		...
		\item A warning is send to the citizens (see \hyperref[table:uc-warn-citizens]{UC-5})
		}\\
...
\caption{UC-\arabic{uc}: Warn citizens in case of an imminent flood}
...
\caption{UC-\arabic{uc}: Warn safety region in case of an imminent flood}
...
\pgfplotstabletypeset[%
	...
	Stakeholders and interests & \compactList{itemize}{%
		...
		\item \textbf{Safety region}: The safety region wants to get updates on the imminent flood
		}\\
	...
	Alternatives &  \compactList{itemize}{
		\item[1a.] 
		\begin{enumerate}
			\item The third party can't connect to the API
			\item The use-case ends
		\end{enumerate}
		}\\
...
\pgfplotstabletypeset[%
	...
	Stakeholders and interests & \compactList{itemize}{%
		...
		\item \textbf{Maintainers}: The maintainers are responsible for repairing/replacing the faulty sensors
		}\\
